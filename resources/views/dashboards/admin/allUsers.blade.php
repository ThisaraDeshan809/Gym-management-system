        ...
        <div class="justify-between flex w-full sm:flex pb-2">
            ...
            <div>
                <a href="{{ route('newUser') }}"
                    class="whitespace-nowrap inline-block px-8 py-2 font-bold text-center uppercase align-middle transition-all bg-transparent border-solid rounded-lg shadow-none cursor-pointer active:opacity-85 leading-pro text-xs ease-soft-in tracking-tight-soft bg-150 bg-x-25 hover:scale-102 active:shadow-soft-xs border border-orange-700 text-orange-500 hover:text-orange-500 hover:opacity-75 hover:shadow-none active:scale-100 active:border-orange-500 active:bg-orange-500 active:text-white hover:active:border-orange-500 hover:active:bg-transparent hover:active:text-orange-500 hover:active:opacity-75"><i
                        class="fas fa-plus mr-2"></i> New User</a>
            </div>
                            ...
                            <tbody>
                                @foreach ($users as $user)
                                    <tr>
                                        ...
                                        <td class="font-semibold leading-tight text-xs">
                                            <div
                                                class="flex flex-wrap gap-4 max-w-48 sm:flex-0 shrink-0 sm:mt-0 sm:ml-auto sm:w-auto">
                                                @foreach ($user->roles as $role)
                                                    @if ($role->name === 'Admin')
                                                        <span
                                                            class="py-2.2 px-3.6 text-xs rounded-1.8 inline-block whitespace-nowrap text-center bg-slate-200 text-slate-650 align-baseline font-bold uppercase leading-none">{{ $role->name }}</span>
                                                    @else
                                                        <span
                                                            class="py-2.2 px-3.6 text-xs rounded-1.8 inline-block whitespace-nowrap text-center bg-cyan-200 text-blue-600 align-baseline font-bold uppercase leading-none">{{ $role->name }}</span>
                                                    @endif
                                                @endforeach
                                            </div>
                                        </td>
