                ...
                <div class="flex flex-wrap items-center justify-center -mx-3">
                    ...
                    <div class="flex max-w-full px-3 mt-4 gap-4 sm:flex-0 shrink-0 sm:mt-0 sm:ml-auto sm:w-auto">
                        @foreach($user->roles as $role)
                            ...
                            @else
                                <span class="py-2.2 px-3.6 text-xs rounded-1.8 inline-block whitespace-nowrap text-center bg-cyan-200 text-blue-600 align-baseline font-bold uppercase leading-none">{{ $role->name }}</span>
                            @endif
                        @endforeach
                    </div>
                    @if($errors->any())
                ...
                <div class="flex-auto pt-0">
                    <form class="relative" id="user-data"  action="{{route( 'updateUser', $user->id )}}" method="post">
                        ...
                        <div active form="info" class="
                            flex flex-col visible w-full h-auto min-w-0 first-letter:break-words bg-white border-0 opacity-100 dark:bg-gray-950 rounded-2xl bg-clip-border">
                            <div class="flex-auto p-6">
                                ...
                                <div class="flex-auto mt-3">
                                    <h5 class="font-bold mb-3 dark:text-white">Access Levels</h5>
                                    ...
                                    <div class="flex justify-between gap-y-2 flex-wrap h-fit w-full">
                                        ...
                                        <div class="pr-2 flex w-full lg:w-4/12">
                                            <div class="w-full relative {{ $errors->has('roles') ? 'border-red-500' : 'border-gray-300' }} flex flex-col h-fit bg-white dark:bg-black dark:text-white p-5 rounded-lg border-0.4 border-gray-300 w-full cursor-pointer">
                                                <label class="">
                                                    <input id="checkbox-{{$role->id}}" name="roles[]" value="{{$role->name}}" {{ in_array($role->name, $userRoles) ? 'checked' : '' }} class="w-5 h-5 ease-soft text-base mr-4 rounded-1.4 checked:bg-gradient-to-tl  checked:from-gray-900 checked:to-slate-800 after:text-xxs after:font-awesome after:duration-250 after:ease-soft-in-out duration-250 relative float-left mt-1 cursor-pointer appearance-none border border-solid border-slate-150 bg-white bg-contain bg-center bg-no-repeat align-top transition-all after:absolute after:flex after:h-full after:w-full after:items-center after:justify-center after:text-white after:opacity-0 after:transition-all after:content-['\f00c'] checked:border-0 checked:border-transparent checked:bg-transparent checked:after:opacity-100" type="checkbox" />
                                                    <label for="checkbox-{{$role->id}}" class="cursor-pointer uppercase select-none dark:text-white text-slate-700">{{$role->name}}</label>
                                                </label>
                                            </div>
                                        </div>
                                        @endforeach
